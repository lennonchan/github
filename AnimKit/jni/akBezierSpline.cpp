...
#else
	//----------------------------------------------------
	//add by zy - scale bezier to avoid outScope
	//[p0], p1,       p2, [p3]

	Ogre::Vector2 pv0 = Ogre::Vector2(p0[0],p0[1]);
	Ogre::Vector2 pv1 = Ogre::Vector2(p1[0],p1[1]);
	Ogre::Vector2 pv2 = Ogre::Vector2(p2[0],p2[1]);
	Ogre::Vector2 pv3 = Ogre::Vector2(p3[0],p3[1]);

	float xLenSimulate = (pv3.x - pv0.x)/2;
	float xLenCull = (pv3.x - pv0.x)*2;
	Ogre::Vector2 pv01 = pv1 - pv0;
	Ogre::Vector2 pv23 = pv3 - pv2;
	
	if(pv01.x > xLenCull)
	{
		pv01 = pv01/fabs(pv01.x)*xLenCull;			
	}

	if(pv23.x > xLenCull)
	{
		pv23 = pv23/fabs(pv23.x)*xLenCull;		
	}
	float xMax = (pv01.x > pv23.x) ? pv01.x : pv23.x;
	if(xMax > xLenSimulate)
	{
		pv01 = pv01/xMax * xLenSimulate;
		pv23 = pv23/xMax * xLenSimulate;
	}
	pv1 = pv0 + pv01;
	pv2 = pv3 - pv23;
	p1[0] = pv1.x; p1[1] = pv1.y;
	p2[0] = pv2.x; p2[1] = pv2.y;
#endif
