...
void gkBlenderSceneConverter::convertObjectGeneral(gkGameObject* gobj, Blender::Object* bobj)
{
	...
	gkMatrix4 invmat = gkMathUtils::getFromFloat(bobj->parentinv);
	

	gkMathUtils::extractTransform(obmat, loc, quat, scale);
	loc = gkVector3(bobj->loc);
	const float PI = 3.1415926;
	gkEuler euler(bobj->rot[0]/PI*180,bobj->rot[1]/PI*180,bobj->rot[2]/PI*180);
	quat = euler.toQuaternion();
	scale = gkVector3(bobj->size);
	
	//loc = gkVector3(bobj->loc);
	//quat = gkQuaternion(bobj->quat);
	//scale = gkVector3(bobj->dscale);
	...
	props.m_transform.invMatrix = invmat;
...
void gkBlenderSceneConverter::convert(bool createGroupInstances)
{
	...
	for (Blender::Base* base = (Blender::Base*)m_bscene->base.first; base; base = base->next)
	{
		...
		if (bobj && bobj->transflag& OB_DUPLIGROUP && bobj->dup_group != 0)
			groups.push_back(bobj);
		else
			convertObject(bobj);
