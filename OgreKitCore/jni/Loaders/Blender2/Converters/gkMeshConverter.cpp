...
#define WIDEN(x,y) WIDEN2(x , y)
#define __WFILE__ WIDEN( __FILE__ , __LINE__ )


#define F_LOG_Float(fp,value) if(fp){ fprintf(fp,#value" = %f \n",value) ;}
#define F_LOG_Int(fp,value) if(fp){ fprintf(fp,#value" = %d \n",value) ;}
#define F_LOG_String(fp,value) if(fp){ fprintf(fp,#value" = %s \n",value.c_str()) ;}
...
void gkBlenderMeshConverter::convertMaterial(Blender::Material* bma, gkMaterialProperties& gma, gkMeshHashKey& hk)
{
	...
	//gkLogMessage("convertTextureFace end");
	...
	gma.m_rblend        = getRampBlendType(bma->rampblend_col);
 	...
 	gkString filePath = "d:\\OgreKit_Materail.log";
 
 	FILE* pWriteFile= fopen(filePath.c_str(),"a+");
 	if (pWriteFile && bma) 
 	{
 		fprintf(pWriteFile,"%s------------%s---------------\n",date,gma.m_name.c_str());
 		F_LOG_String(pWriteFile,gma.m_name);
 		F_LOG_Int(pWriteFile,bma->har);
 		F_LOG_Float(pWriteFile,bma->ref);
 		F_LOG_Float(pWriteFile,bma->emit);
 		F_LOG_Float(pWriteFile,bma->amb);
 		F_LOG_Float(pWriteFile,bma->spec);
 		F_LOG_Float(pWriteFile,bma->alpha);
 		F_LOG_Float(pWriteFile,bma->r);
 		F_LOG_Float(pWriteFile,bma->g);
 		F_LOG_Float(pWriteFile,bma->b);
 		F_LOG_Float(pWriteFile,bma->specr);
 		F_LOG_Float(pWriteFile,bma->specg);
 		F_LOG_Float(pWriteFile,bma->specb);
 		F_LOG_Int(pWriteFile,bma->mode);
 		F_LOG_Float(pWriteFile,bma->b);
 		F_LOG_Float(pWriteFile,bma->translucency);
 		F_LOG_Float(pWriteFile,bma->mirb);
 		F_LOG_Float(pWriteFile,bma->mirg);
 		F_LOG_Float(pWriteFile,bma->mirr);
 		F_LOG_Float(pWriteFile,bma->ray_mirror);
 		F_LOG_Float(pWriteFile,bma->ambr);
 		F_LOG_Float(pWriteFile,bma->ambg);
 		F_LOG_Float(pWriteFile,bma->ambb);
 		F_LOG_Float(pWriteFile,bma->ang);
 		F_LOG_Float(pWriteFile,bma->darkness);
 		F_LOG_Int(pWriteFile,bma->dynamode);
 		F_LOG_Int(pWriteFile,bma->mapto);
 		F_LOG_Int(pWriteFile,bma->mapto_textured);
 		F_LOG_Int(pWriteFile,bma->material_type);
 		F_LOG_Int(pWriteFile,bma->ramp_show);
 		...
 		{ 
			...
			if (!mtex) continue;
  			...
  			if (!ima)
  				continue;
  			gkString ImageName = GKB_IDNAME(ima);
			fprintf(pWriteFile,"------------%s---------------\n",ImageName.c_str());
  			F_LOG_String(pWriteFile,ImageName);
  
			if (mtex->tex)
			{
				if (mtex->tex->imaflag & TEX_USEALPHA)
				{
					F_LOG_Int(pWriteFile,TEX_USEALPHA);
				}
			}

   			if (mtex->texflag & MTEX_STENCIL)
   			{
   				F_LOG_Int(pWriteFile,MTEX_STENCIL);
   			}
   			if (mtex->texflag & MTEX_NEGATIVE) F_LOG_Int(pWriteFile,MTEX_NEGATIVE);
   				
   			if (mtex->texflag &  MTEX_RGBTOINT) F_LOG_Int(pWriteFile,MTEX_RGBTOINT);
   				
   			if (mtex->mapto & MAP_ALPHA) F_LOG_Int(pWriteFile,MAP_ALPHA);

			if (mtex->mapto & MAP_COL) F_LOG_Int(pWriteFile,MAP_COL);
   
   			if ((mtex->mapto & MAP_NORM) || (mtex->maptoneg & MAP_NORM)) F_LOG_Int(pWriteFile,MAP_NORM);
   			if ((mtex->mapto & MAP_SPEC) || (mtex->maptoneg & MAP_SPEC)) F_LOG_Int(pWriteFile,MAP_SPEC);
   				
   			if ((mtex->mapto & MAP_REF)  || (mtex->maptoneg & MAP_REF)) F_LOG_Int(pWriteFile,MAP_REF);
   			
   			if ((mtex->mapto & MAP_EMIT) || (mtex->maptoneg & MAP_EMIT)) F_LOG_Int(pWriteFile,MAP_EMIT);
   				
   
   			if (mtex->normapspace == MTEX_NSPACE_OBJECT)
   			{
   				F_LOG_Int(pWriteFile,mtex->normapspace);
   			} 
			F_LOG_Float(pWriteFile,mtex->norfac) ;
			F_LOG_Float(pWriteFile,mtex->colfac) ;
			F_LOG_Float(pWriteFile,mtex->alphafac) ;
			F_LOG_Float(pWriteFile,mtex->colspecfac) ;
			F_LOG_Float(pWriteFile,mtex->hardfac) ;
  		  }
 		}
 		fclose(pWriteFile);
 		pWriteFile = NULL;	
	...
	if (bma->mode & TF_TWOSIDE)  gma.m_mode |= gkMaterialProperties::MA_TWOSIDE;
...
// modify by dabao 2012.08.06 -----------------------------------------------------------------------
				if (mtex->mapto & MAP_ALPHA)
				{
					gte.m_mode |= gkTextureProperties::TM_ALPHA;
                    gte.m_mode |= gkTextureProperties::TM_ALPHA_USE;
					gma.m_mode |= gkMaterialProperties::MA_ALPHABLEND;
				}

				if (mtex->mapto & MAP_COL)
				{
					gte.m_mode |= gkTextureProperties::TM_COLOR_USE;
				}
				...
				if (mtex->tex)
				{
					if (mtex->tex->imaflag & TEX_USEALPHA)
					{
						gte.m_mode |= gkTextureProperties::TM_TEXTURE_ALPHA_USE;
					}
				}
// modify end
//----------------------------------------------------------------------------------------------------

				// add by dabao 2012-12-10  for texture Matrix
				{
					gte.m_Transform.setIdentity();
					gkVector3 position(mtex->ofs);
					gkVector3 scale(mtex->size);
					gkQuaternion rot;
					gte.m_Transform.set(position,rot,scale);
				}
				...
				gte.m_color = gkColor(mtex->colfac,mtex->colfac,mtex->colfac,mtex->alphafac);
			}
		}
	}
     //gkLogMessage("gkBlendFileTemplate begin");
	// add by dabao 2012.08.06  Test Material
	//gkBlendFileTemplate* tempFile = new gkBlendFileTemplateImpl(&gma);
	//gkBlendFile2Ogre::OgreMaterai->ParseFile(tempFile);
	//delete tempFile;
     //gkLogMessage("gkBlendFileTemplate end");
}
