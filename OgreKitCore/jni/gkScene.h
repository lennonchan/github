...
public:
	...
	// add by dabao 2012-11-21  create sub ViewPort for clip
	Ogre::Viewport* CreateSubScene(Ogre::Camera * activeCamera,int ZOrder = 0, float left = 0.0f, float top = 0.0f ,
		float width = 1.0f, float height = 1.0f);

	// add by dabao 2012-11-27  delete sub ViewPort for visible
	bool DeleteSubScene(Ogre::Viewport * subViewPort);

	// add by dabao 2012-12-20  get camera window size by Z of Camera Order
	gkVector2 getViewWindowHightAndWidthByCamera_Z(gkVector3 position);

	// add by dabao 2012-12-21  force update ogre all animate
	void forceUpdateAllOgreAnimate();
	void forceUpdateOgreAnimate(gkString animateState);

	// add by dabao 2012-12-24  get ray test all gkGameObjects
	gkGameObjectArray isMouseClicked(); 
	// add by dabao 2012-12-26  get ogre ray test object
	Ogre::RaySceneQueryResult& getOgreRayTest();
