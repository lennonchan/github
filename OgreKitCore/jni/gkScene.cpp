...
void gkScene::_applyBuiltinParents(gkGameObjectSet& instanceObjects)
{
	...
	while (it.hasMoreElements())
	{
		...
		{
			gkTransformState s;
			s.loc =  gobj->getProperties().m_transform.loc;
			s.loc_enable = true;
			gkMatrix4 mat = gobj->getParentInvTransform(s);
			gobj->getNode()->setPosition(mat.getTrans());
		}
...
//************************************
// Method:    CreateSubScene
// FullName:  gkScene::CreateSubScene
// Access:    public 
// Returns:   Ogre::Viewport*
// Qualifier:
// Parameter: Ogre::Camera * activeCamera
// Parameter: int ZOrder
// Parameter: float left   window screen left
// Parameter: float top    window screen top
// Parameter: float width  [0.0 -- 1.0]
// Parameter: float height [0.0 -- 1.0]
//************************************
Ogre::Viewport* gkScene::CreateSubScene(Ogre::Camera * activeCamera,int ZOrder, float left, float top,
										float width, float height)
{
	Ogre::RenderWindow * MainWindow = gkWindowSystem::getSingleton().getMainRenderWindow();

	Ogre::Viewport* subViewPort = NULL;

	if (MainWindow && activeCamera)
	{
		subViewPort = MainWindow->addViewport(activeCamera,ZOrder,left,top,width,height);
	}
...
//************************************
// Method:    DeleteSubScene
// FullName:  gkScene::DeleteSubScene
// Access:    public 
// Returns:   bool
// Qualifier:
// Parameter: Ogre::Viewport * subViewPort not null
//************************************
bool gkScene::DeleteSubScene(Ogre::Viewport * subViewPort)
{
	if (subViewPort)
	{
		Ogre::RenderWindow * MainWindow = gkWindowSystem::getSingleton().getMainRenderWindow();
		if (MainWindow)
		{
			int ViewPortCount =  MainWindow->getNumViewports();
			for (int i =0; i< ViewPortCount ; ++i)
			{
				Ogre::Viewport* tempDeleteRef = MainWindow->getViewport(i);

				if (subViewPort == tempDeleteRef)
				{
					MainWindow->removeViewport(tempDeleteRef->getZOrder());
					return true;
				}
			}
		}
...
gkVector2 gkScene::getViewWindowHightAndWidthByCamera_Z(gkVector3 position)
{
	Ogre::RenderWindow * MainWindow = gkWindowSystem::getSingleton().getMainRenderWindow();
	if (MainWindow)
	{
		Ogre::Viewport* mainViewPort = MainWindow->getViewport(0);
		if (mainViewPort)
		{
			gkVector2 nearClipSize = gkVector2(mainViewPort->getActualWidth(),mainViewPort->getActualHeight());
			Ogre::Camera * mainCamera = mainViewPort->getCamera();
			if (mainCamera)
			{
				Ogre::Matrix4 viewMat = mainCamera->getViewMatrix(true);
				gkVector3 viewPosition = viewMat * position;
				gkScalar radio = viewPosition.z / mainCamera->getNearClipDistance();
				return nearClipSize * radio;
			}
		}
	}
...
void gkScene::forceUpdateAllOgreAnimate()
{
	if (m_manager)
	{
		m_manager->_applySceneAnimations();
	}
}

void gkScene::forceUpdateOgreAnimate(gkString animateState)
{
	if (m_manager)
	{
		Ogre::AnimationState* targetState = m_manager->getAnimationState(animateState);
		if (targetState)
		{
			Ogre::Animation* targetAnimate = m_manager->getAnimation(targetState->getAnimationName());
			if (targetAnimate)
			{				
				Ogre::Animation::NodeTrackIterator nodeTrackIt = targetAnimate->getNodeTrackIterator();
				while(nodeTrackIt.hasMoreElements())
				{
					Ogre::Node* nd = nodeTrackIt.getNext()->getAssociatedNode();
					if (nd)
						nd->resetToInitialState();
				}
				
				Ogre::Animation::NumericTrackIterator numTrackIt = targetAnimate->getNumericTrackIterator();
				while(numTrackIt.hasMoreElements())
				{
					const Ogre::AnimableValuePtr& animPtr = numTrackIt.getNext()->getAssociatedAnimable();
					if (!animPtr.isNull())
						animPtr->resetToBaseValue();
				}

				targetAnimate->apply(targetState->getTimePosition(),targetState->getWeight());
			}
		}
	}
}

Ogre::RaySceneQueryResult & gkScene::getOgreRayTest()
{
	static Ogre::RaySceneQuery *m_rayQuery = NULL;
	Ogre::Camera * oc = getMainCamera()->getCamera();
	...
	gkVector2 OgreMousePosition(ncx,ncy);
	Ogre::Ray dest;
	if(oc) 
	{
		oc->getCameraToViewportRay(OgreMousePosition.x, OgreMousePosition.y, &dest);
	}
	if (m_rayQuery == 0)
	{
		Ogre::SceneManager* mgr = getManager();
		m_rayQuery = mgr->createRayQuery(dest);
	}
	else m_rayQuery->setRay(dest);

	return m_rayQuery->execute();
}

gkGameObjectArray gkScene::isMouseClicked()
{
	gkGameObjectArray RayHitObjects;
	Ogre::RaySceneQueryResult & res = getOgreRayTest();
	bool result = false;
	for (Ogre::RaySceneQueryResult::iterator it = res.begin(); it != res.end(); ++it)
	{
		Ogre::RaySceneQueryResultEntry ent = (*it);

		gkGameObjectSet& instaneceObject = getInstancedObjects();

 		gkGameObjectSet::Iterator Objectit = instaneceObject.iterator();
 		while (Objectit.hasMoreElements())
 		{
 			gkGameObject* gobj = Objectit.getNext();
 
 			if (gobj 
 				&& ent.movable == gobj->getNode()->getAttachedObject(0)
 			   )
 			{
 				RayHitObjects.push_back(gobj);
 			}
 		}
	}
	return RayHitObjects;
}
